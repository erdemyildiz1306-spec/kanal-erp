...
export default function TrendyolInvoicesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/trendyol/invoices/pending", { cache: "no-store" });
  ...
  const runIssue = async (orderId: string, mode: "efaturam" | "link" | "file", extra?: FormData) => {
    ...
    try {
      let res: Response;
      if (mode === "file" && extra) {
        res = await fetch("/api/trendyol/invoices/upload-file", {
          method: "POST",
          credentials: "include",
          body: extra,
        });
      } else if (mode === "link") {
        res = await fetch("/api/trendyol/invoices/send-link", {
          method: "POST",
          credentials: "include",
        ...
        res = await fetch("/api/trendyol/invoices/issue", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      setBanner({
        kind: "ok",
        text: `Fatura gönderildi: ${data.invoiceNumber || data.orderNumber || orderId}`,
      });
      setLinkModal(null);
      setFileModal(null);
      setLinkValue("");
  ...
  if (loading) return <Spinner label="Trendyol faturaları yükleniyor…" />;

  return (
    <div className="erp-page max-w-6xl mx-auto space-y-6">
      <PageHeader
        title="Trendyol Fatura"
        subtitle="E-Faturam / e-Arşiv kesip Trendyol'a fatura linki veya dosyası gönderin"
        action={
      ...
      <div className="erp-card p-4 text-sm text-slate-600 space-y-1">
        <p>
          ...
          ,{" "}
          <a
            href="https://developers.trendyol.com/reference/uploadinvoicefile.md"
            className="text-orange-700 underline"
            target="_blank"
            rel="noreferrer"
          >
      ...
      <div className="erp-card overflow-hidden">
        ...
        {orders.length === 0 ? (
          <p className="p-8 text-center erp-muted">Fatura bekleyen Trendyol siparişi yok.</p>
        ) : (
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o._id} className="border-t border-slate-100 hover:bg-slate-50/80">
                    ...
                    <td className="py-3 px-4 font-mono text-xs">{o.packageId || "—"}</td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex justify-end flex-wrap gap-1">
                        <button
                          ...
                          title="E-Faturam ile kes ve Trendyol'a gönder"
                        >
                          <FileText size={14} />
                          E-Faturam
                        </button>
      ...
      >
        <div className="space-y-3">
          <label className="block text-sm">
            e-Arşiv / e-Fatura HTTPS linki (10 yıl erişilebilir olmalı)
            <input
              type="url"
      ...
      >
        <div className="space-y-3">
          ...
          />
          <label className="block text-sm">
            Fatura no (mikro ihracat için zorunlu)
            <input
              className="mt-1 w-full rounded-lg border px-3 py-2 font-mono text-xs"
              value={invoiceNo}
              onChange={(e) => setInvoiceNo(e.target.value)}
